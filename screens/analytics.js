...
import {
  updateAnalytics,
  retrieveMonthlyAnalytics,
  retrieveYearlyAnalytics,
} from "../firestoreUtils";
import { Button, Switch, TextInput } from "react-native-paper";
import { BarChart, LineChart } from "react-native-chart-kit";
import {
  FetchCompletions,
  getRestaurantTipsMonthly,
  getRestauranTipsYearly,
  promptGPT,
} from "../openaiConfig";
  ...
  useEffect(() => {
    const calculateMonthly = async () => {
      try {
        ...
        if (totalFoodSaved && prevMonth) {
          const change = (
            ((totalFoodSaved - prevMonth) / prevMonth) *
            100
          ).toFixed(1);
          setChange(change);
        }

        const results = Object.keys(monthly).filter(
          (day) => day !== "counter" && day !== "prevCounter"
        );
        ...
        const averageFoodSavedPerDay = (
          totalFoodSaved / Object.keys(results).length
        ).toFixed(1);
        const daysWithNoFoodSaved = Object.keys(filteredMonthly).filter(
          (day) => filteredMonthly[day] === 0
        ).length;
  ...
  useEffect(() => {
    const calculateYearly = async () => {
      try {
        ...
        const resultsYearly = Object.keys(yearly).filter(
          (month) => month !== "counter"
        );
        ...
        const averageFoodSavedPerMonth = (
          totalFoodSavedYear / Object.keys(resultsYearly).length
        ).toFixed(1);
        const monthWithNoFoodSaved = Object.keys(yearly).filter(
          (month) => yearly[month] === 0
        );
      ...
      <KeyboardAwareScrollView style={{ flex: 1 }} behavior="padding">
        <View style={styles.topContainer}>
          {filteredMonthly && selected === "monthly" ? (
            <View>
              <Text style={styles.title}>Monthly report - June</Text>
            ...
            <View style={{ flex: 1 }}>
              <Text style={styles.title}>Yearly report - 2024</Text>
              {/* <Text style={styles.title}>
        ...
        <View style={styles.analyticsContainer}>
          <View style={styles.midContainer}>
              ...
              {filteredMonthly && selected === "monthly" ? (
                <View style={styles.dataContainer}>
                  ...
                  {change > 0 ? (
                    <>
                      ...
                      >
                        You have saved {change}% more food this month compared
                        to last month!
                      </Text>
                    </>
                  ) : (
                    ...
                    >
                      You have saved {change}% less food this month compared to
                      last month
                    </Text>
                  )}
                  <>
                    <Text style={styles.subtitle}>
                      Total food saved this month: {totalFoodSavedMonth}
                    </Text>
                    <Text style={styles.subtitle}>
                      Average food saved per day: {averageFoodSavedPerDay}
                    </Text>
                    <Text style={styles.subtitle}>
                ...
                <View style={styles.dataContainer}>
                  ...
                  <>
                    ...
                    <Text style={styles.subtitle}>
                      Average food saved per month: {averageFoodSavedPerMonth}
                    </Text>
                    <Text style={styles.subtitle}>
                      Days with no food saved: {monthWithNoFoodSaved.length}
                    </Text>
                  </>
                </View>
              ) : (
            ...
            {filteredMonthly && selected === "monthly" ? (
              ...
              <Text style={styles.title}>
                AI is currently analysing your data...
              </Text>
            )}
            <View style={styles.chatbotContainer}>
              <View style={styles.chatbotHeader}>
                ...
                >
                  I can answer climate change related questions or anything
                  about food waste!
                </Text>
              </View>
              ...
              <View style={styles.promptContainer}>
                ...
                <Button
                  ...
                  onPress={handleSend}
                  labelStyle={{ color: "black", alignSelf: "center" }}
                >
                  Send
                </Button>
              </View>
